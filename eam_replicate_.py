"""This module reports on repeating sets of characters."""
...
def main():
    ...
    try:
        ...
        while True:
            try:
                user_input = input(
                    "\nInput Y for clipboard, YYMMDD for specific date, "
                     "or Enter to terminate: "
                     ).strip()
                ...
                print(f"ValueError in main loop: {e}")
                continue
            except pyperclip.PyperclipException as e:
                print(f"Clipboard error in main loop: {e}")
                continue
